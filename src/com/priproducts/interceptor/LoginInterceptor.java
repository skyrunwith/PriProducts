package com.priproducts.interceptor;
...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if(!(url.contains("/index") || url.contains("/login") || url.contains("/login")
                || url.contains("/register")|| url.contains("/searchShangcheng") || url.contains("/shangcheng"))){
            HttpSession session=request.getSession();
            User user=(User) session.getAttribute("user");
            if(user==null){
                System.out.println("--用户----拦截了，请先登录----");
                request.getRequestDispatcher("/shouye/login.jsp").forward(request, response);
            }else{
                System.out.println("--用户----已登录，放行----");
                return true;
            }
        }else{
        	return true;
